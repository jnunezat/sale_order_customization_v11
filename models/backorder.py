    ...
    def _compute_date_prev(self):
        ...
        for line in self:
            if line.backorder_id and line.product_id:
                back_to_lines.setdefault(line.backorder_id.id, []).append(line)

        for _, lines in back_to_lines.items():
            ...
            rows = Pol.read_group(
                domain=[
                    ...
                    ('date_planned', '>=', bo.date),
                ],
                fields=['product_id', 'date_planned:min'],
                groupby=['product_id'],
                lazy=False,
            )
            ...
            for l in lines:
                ...
                if dt:
                    l.date_prev = dt
                    key = (l.product_id.id, dt)
                    if key not in qty_cache:
                        pol = Pol.search([
                            ('order_id.state', '=', 'purchase'),
